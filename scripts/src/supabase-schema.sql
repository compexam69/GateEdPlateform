...
create table if not exists quiz_questions (
  ...
  question_type      text    not null default 'SCQ' check (question_type in ('SCQ', 'MCQ', 'NAT')),
  options            jsonb,                              -- {"A":"...", "B":"..."}; null for NAT
  correct_answer     text    not null,                  -- SCQ: "A"/"B"/"C"/"D"; MCQ: "A,C"; NAT: "9.8"
  explanation        text,
...
-- ============================================================
-- SECTION 28: MCQ / NAT question_type migration
-- Run in Supabase SQL Editor if upgrading from a version that
-- only supported SCQ (single-choice) questions.
-- Safe to re-run (all statements are idempotent).
-- ============================================================

-- 1. Add question_type column (defaults to 'SCQ' for all existing rows)
alter table quiz_questions
  add column if not exists question_type text not null default 'SCQ'
  check (question_type in ('SCQ', 'MCQ', 'NAT'));

-- 2. Make options nullable — NAT questions have no A/B/C/D options
alter table quiz_questions
