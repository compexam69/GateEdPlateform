...
export default function Hard75DietPage() {
  ...
  // Build a simple 30-day calendar from daily logs
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: "Success Rate",  value: `${followed + missed > 0 ? Math.round((followed / (followed + missed)) * 100) : 0}%`, color: "text-primary" },
        ...
        {/* Today */}
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Today — {format(new Date(), "MMMM d")}</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {todayDiet && (
              <div className={cn("flex items-center gap-2 p-3 rounded-lg", todayDiet.followed ? "bg-green-500/10 text-green-600" : "bg-destructive/10 text-destructive")}>
                {todayDiet.followed
                  ? <CheckCircle2 className="w-4 h-4 shrink-0" />
                  : <XCircle className="w-4 h-4 shrink-0" />
                }
                <span className="text-sm font-medium">{todayDiet.followed ? "Diet followed today" : "Diet not followed"}</span>
              </div>
            )}
            <div className="flex gap-3">
              <Button
                className={cn("flex-1", todayDiet?.followed === true && "ring-2 ring-green-500")}
              ...
              <Button variant="outline"
                className={cn("flex-1", todayDiet?.followed === false && "ring-2 ring-destructive")}
        ...
        {/* Rules */}
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Salad className="w-4 h-4 text-primary" /> 75 Hard Diet Rules</CardTitle></CardHeader>
          <CardContent>
        ...
        {/* History heatmap (last 30 from daily logs) */}
        {dietLogs.length > 0 && (
          <Card>
                ...
                {dietLogs.map((l: any) => (
                  <div
                    ...
                    className={cn(
                      ...
                      l.diet_done ? "bg-green-500/20 text-green-600" : "bg-destructive/10 text-destructive"
                    )}
                  >
